...
def train(model, graph, x, y, loss_func, optimizer, mean=None, gamma=0):
    model.train()
    output, Y = model(graph, x, mean=mean, gamma=gamma)
...
@torch.no_grad()
def evaluate(model, graph, x, y, mean=None, gamma=0):
    model = model.eval()
    output, Y = model(graph, x, mean=mean, gamma=gamma)
    output = output[: y.shape[0]]
...
def parse_args():
    ...
    parser.add_argument("--dataset", type=str, default="igb-small")
    ...
    parser.add_argument("--lmbd", type=float, default=4)
    ...
    parser.add_argument("--inp_dropout", type=float, default=0)
    ...
    parser.add_argument("--activate", type=int, default=1)
...
def main():
    ...
    print(dataset[0], file=sys.stderr)
    mem1 = process.memory_info().rss / (1024 * 1024 * 1024)
    print("Graph total memory:", mem1 - mem, "GB", file=sys.stderr)

    g, feats, labels, n_classes, splitted_idx = dataset
    output_channels = n_classes

    input_channels = feats.shape[1]
    print("Feature dimension:", input_channels, file=sys.stderr)
    ...
    times_full = torch.zeros(num_nodes, dtype=torch.int)
    ...
    for e in range(args.epochs):
        ...
        for i in tqdm(sequence):
            ...
            loss, output = train(model, graph, x, y, loss_func, optimizer, mean, args.gamma)
            # torch.cuda.synchronize()
        ...
        for i, (input_nodes, output_nodes, graph) in enumerate(tqdm(valid_loader)):
            input_nodes = input_nodes.to(torch.device('cpu'))
            output_nodes = output_nodes.to(torch.device('cpu'))
            ...
            sub_to_full = graph.ndata['_ID'].to('cpu')
            mean = mean_full[sub_to_full].to(device)
            correct, tot, output = evaluate(model, graph, x, y, mean, args.gamma)
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(best_model, f'{args.dataset}-{args.alpha}-{args.gamma}.pt')
