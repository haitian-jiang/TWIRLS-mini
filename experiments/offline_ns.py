...
def train(model, graph, x, y, loss_func, optimizer):
    model.train()
    output = model(graph, x)[graph.split_idx]
...
def parse_args():
    ...
    parser.add_argument("--dir", type=str, default="/nvme4n1/offline_subgraph")
...
def graph_load(args, queue, sequence, fanout_info, feats: torch.Tensor, labels: torch.Tensor, label_offset, device):
    for i in sequence:
        subgraph: dgl.DGLGraph = torch.load(f"{args.dir}/{args.dataset}-{fanout_info}-ns/train-{i}.pt")
        x = feats[subgraph.ndata[dgl.NID]].float().to(device, non_blocking=True)
        y = labels[subgraph.ndata[dgl.NID][subgraph.split_idx] - label_offset].long().to(device, non_blocking=True)
        subgraph = subgraph.to(device, non_blocking=True)
        subgraph.split_idx = subgraph.split_idx.to(device, non_blocking=True)
        queue.put((subgraph, x, y))
...
def main():
    ...
    for e in range(args.epochs):
        ...
        for i in tqdm(sequence):
            ...
            # torch.cuda.synchronize()
            # tic = time.time()
            loss = train(model, subgraph, x, y, loss_func, optimizer)
            # torch.cuda.synchronize()
        ...
        # for i, (input_nodes, output_nodes, blocks) in enumerate(tqdm(valid_loader)):
        #     eids = torch.cat([block.edata[dgl.EID] for block in reversed(blocks)])
        #     subgraph = dgl.edge_subgraph(g, eids, relabel_nodes=True)
        #     orig_nodes_id = subgraph.ndata[dgl.NID].cpu()
        #     rev_idx = find_indices_in(output_nodes.cpu().numpy(), orig_nodes_id.numpy())
        ...
        #     x = feats[orig_nodes_id].float().to(device)
        #     y = labels[orig_nodes_id[subgraph.split_idx] - label_offset].long().to(device)
        #     subgraph = subgraph.to(device)
        #     subgraph.split_idx = subgraph.split_idx.to(device)
        #     correct, tot = evaluate(model, subgraph, x, y)
        #     valid_correct += correct
        #     valid_tot += tot
        # val_acc = valid_correct / valid_tot
        # # --- test --- #
        test_correct, test_tot, test_acc = 0, 0, 0
        # for i, (input_nodes, output_nodes, blocks) in enumerate(tqdm(test_loader)):
        #     eids = torch.cat([block.edata[dgl.EID] for block in reversed(blocks)])
        #     subgraph = dgl.edge_subgraph(g, eids, relabel_nodes=True)
        #     orig_nodes_id = subgraph.ndata[dgl.NID].cpu()
        #     rev_idx = find_indices_in(output_nodes.cpu().numpy(), orig_nodes_id.numpy())
